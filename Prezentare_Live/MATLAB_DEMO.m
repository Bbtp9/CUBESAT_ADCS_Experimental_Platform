...
for k = 1:max_detumble_iterations
    ...
    if numel(data) == 11 && all(~isnan(data))
        ...
        phiM=data(11);             % grade
        % sz = csi * (sind(phiZ) + dt * gz * 180 / pi) + (1 - csi) * sind(phiM);
        % 
        % cz = csi * (cosd(phiZ) + dt * gz * 180 / pi) + (1 - csi) * cosd(phiM);

        % phiZ = csi * (phiZ + dt * gz * 180 / pi) + (1 - csi) * phiM;
        % phiZ = atan2d (sz, cz);
        phiZ = phiM;
       ...
       % De incercat de pus unghiurile la timpii lor pe grafic: inlocuieste
       % if ul de sus cu asta de jos si decomenteaza liniile 107-109 si
       % legenda de la 114 cu liniile 116,117,118

       % if abs(phiZ - memphiZ) < 180
       %    addpoints(h_unghi, t, phiZ);
       %    addpoints(h_phi_cmd, t, phi_cmd);
       % else
       %    addpoints(h_unghi, t, NaN);
       %    addpoints(h_phi_cmd, t, phi_cmd);
       % end
